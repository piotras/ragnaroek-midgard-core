...
MidgardReflectionProperty *midgard_test_property_reflector_create(MgdObject *object)
{
	...
	return midgard_reflection_property_new(klass);
}
...
void midgard_test_property_reflector_check_is_link(MgdObjectTest *mot, gconstpointer data)
{
	...
	for (i = 0; i < n_prop; i++) {
		...
		MidgardReflectionProperty *tmrp = midgard_reflection_property_new(klass);
		g_assert(tmrp != NULL);
...
void midgard_test_property_reflector_check_is_multilang(MgdObjectTest *mot, gconstpointer data)
{
	...
	for (i = 0; i < n_prop; i++) {
		...
		ismultilang = midgard_reflection_property_is_multilang(mrp, pspecs[i]->name);

		if (!ismultilang)
			continue;

		MidgardObjectClass *klass = MIDGARD_OBJECT_GET_CLASS(object);
		gboolean class_is_multilang = midgard_object_class_is_multilang(klass);

		g_assert(class_is_multilang != FALSE);
	}
