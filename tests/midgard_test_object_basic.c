...
{
	MgdObject *object = midgard_object_new(mgd->mgd, name, value);
...
void midgard_test_object_basic_purge(MgdObjectTest *mot, gconstpointer data)
{
	...
	while(objects[i] != NULL) {
		...
		MgdObject *purged_object = midgard_object_new(mgd->mgd, G_OBJECT_TYPE_NAME(mot->object), NULL);
...
void midgard_test_object_basic_create_run(void)
{
	...
	MidgardUser *user = midgard_user_auth(mgd, "root", "password", NULL, FALSE);
	g_assert(user != NULL);
...
void midgard_test_object_basic_lock(MgdObjectTest *mot, gconstpointer data)
{
	...
	gchar *revised = NULL;
	g_object_get(object->metadata, "revised", &revised, NULL);
