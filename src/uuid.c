 ...
 * \ingroup uuid
 *
 * The predefined Midgard namespace UUID. Expressed in binary format
...
gchar *midgard_uuid_external(const gchar *external) {
        ...
        _MGD_MD5_CTX md5;
        _MGD_MD5Init(&md5);
        _MGD_MD5Update(&md5, namespace_uuid, sizeof(namespace_uuid));
        _MGD_MD5Update(&md5, (unsigned char *) external, strlen(external));
        _MGD_MD5Final(uuid, &md5);
