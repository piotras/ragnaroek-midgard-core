		...
		MidgardQueryBuilder *builder, const gchar *name, const gchar *dir)
{
	...
	if(!_midgard_core_qb_parse_property(builder, &(order->constraint), name)) {

		midgard_core_query_order_free(order);
		return FALSE;
	}

	_midgard_core_qb_add_table(builder, order->constraint->priv->prop_left->table);
...
gchar *midgard_core_query_order_get_property_sql(MidgardQueryOrder *order)
{
        g_assert(order != NULL);
	...
	g_string_append_printf(cond,
			"%s %s",
			g_str_has_prefix (order->constraint->priv->prop_left->field, "metadata_") ? order->constraint->priv->prop_left->field : order->constraint->priv->pspec->name, order->constraint->priv->order_dir);
