/* $Id: format.c 15748 2008-03-18 11:37:50Z piotras $
 *
...
int mgd_parser_addint(mgd_parser * parser, long num)
{
	long begin, end;
	char tmp;
	if (num < 0) {
...
{
	return mgd_parser_addint(parser, (long) data);
}
			      ...
			      int need_qp)
{
	...
	pool = mgd_alloc_pool();
	parser = (mgd_parser *) mgd_alloc(pool, sizeof (mgd_parser)); 
	parser->pool = pool;
	parser->next = NULL;
...
void mgd_parser_free_all()
{
	...
	/* DO NOT FREE parser and its members!
	 * Those already are owned by pool manager. */ 
	/*
	 * g_free(parser->name);
...
char *mgd_vformat_ext(mgd_parser * parser, midgard_pool * pool, const char *fmt, va_list args)
{
	char *str;
	mgd_parser_callback_info *callback;
		...
		if (*fmt == '$') {
			if (*(fmt + 1) == '$') {
				...
				switch (*(fmt + 1)) {
					case 'd':
					case 'i':
						mgd_parser_std_d(parser, (void *) ((long)va_arg(args, int)));
						fmt += 2;
					break;
					case 'q':
						mgd_parser_std_q(parser, (void *) va_arg(args, charp));
						fmt += 2;
					break;
					case 'D':
						mgd_parser_std_D(parser, (void *) va_arg(args, intp));
						fmt += 2;
					break;
					default:
						callback = &parser->callbacks[(int) (*(fmt + 1))];
						if (callback->func) 
						switch (callback->ptype) {
							case MGD_CHAR:
							case MGD_INT:
								callback->func(parser, (void *) ((long)va_arg(args,int)));
								fmt += 2;
							break;
							case MGD_INTPTR:
								callback->func(parser, (void *) va_arg(args, intp));
								fmt += 2;
							break;
							case MGD_STR:
								callback->func(parser, (void *) va_arg(args, charp));
								fmt += 2;
							break;
							case MGD_PTR:
								callback->func(parser, (void *) va_arg(args, voidp));
								fmt += 2;
							break;
						} else
...
char *mgd_vformat(midgard * mgd, midgard_pool * pool, const char *fmt, va_list args)
{
	char *str;
	va_list qa;
	...
	if(args) {
		...
		va_copy(qa, args);
		str = mgd_vformat_ext(mgd->parser, pool, fmt, qa);
		va_end(qa);
		
	} else {
		
		str = mgd_vformat_ext(mgd->parser, pool, fmt, args);
	}
